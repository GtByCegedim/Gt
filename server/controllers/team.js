...
const addUserInTeam = async (req, res, next) => {
  try {
    ...
    if (findTeamById.manager != manager.id)
      return next(new apiError("You are not manager of this project", 401));
    ...
    if (!AddUserInTeam)
      return next(new apiError("error added user in team", 401));
...
const baneTeam = async (req, res, next) => {
  try {
    ...
    const findTeamById = await Team.findByPk(team_id);
    if (!findTeamById) return next(new apiError("No team found", 404));
    if (findTeamById.manager != manager_id)
      return next(new apiError("You are not manager of this Team", 401));
