  ...
  <div class="flex-1 p-6">
    ...
    <div class="p-6">
      <table class="w-full text-left table-collapse">
        <thead>
          <tr>
            <th class="text-sm font-medium p-2 bg-gray-100 text-gray-600">#</th>
        ...
        <tbody>
          <tr v-for="(project, index) in projects" :key="project.id">
            ...
            <td class="p-2 border-t">{{ project.projectNumber }}</td>
            <td class="p-2 border-t">
  <template v-for="member in project.members.split(', ')">
    <div class="bg-gray-300 text-gray-700 rounded-full inline-block p-2 hover:bg-gray-400" v-tooltip="member">{{ member.charAt(0) + member.charAt(member.length - 1) }}</div>
  </template>
  <div class="bg-gray-300 text-gray-700 rounded-full inline-block p-2 hover:bg-gray-400" v-tooltip="'Add Member'">
